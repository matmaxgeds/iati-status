...
class TestIATIDatastore(WebTestBase):
    """
    Test datastore (datastore.codeforiati.org)
    """
    requests_to_load = {
        'datastore.codeforiati.org': {
            'url': 'http://datastore.codeforiati.org/'
        },
        'Activities updated since 2 days ago': {
            'url': 'http://datastore.codeforiati.org/api/1/access/activity.xml?limit=0&last-change__gt=' + str(date.today() - timedelta(days=2)),
            'min_response_size': 295
        },
        'Activities updated since 3 days ago': {
            'url': 'http://datastore.codeforiati.org/api/1/access/activity.xml?limit=0&last-change__gt=' + str(date.today() - timedelta(days=3)),
            'min_response_size': 295
        },
        'Datastore download: CSV': {
            'url': 'http://datastore.codeforiati.org/api/1/access/activity.csv'
        },
        'Datastore download: XML': {
            'url': 'http://datastore.codeforiati.org/api/1/access/activity.xml'
        },
        'Datastore download: JSON': {
            'url': 'http://datastore.codeforiati.org/api/1/access/activity.json'
        },
        'Datastore list of datasets': {
            'url': 'http://datastore.codeforiati.org/api/1/about/datasets/fetch_status'
        }
    }
